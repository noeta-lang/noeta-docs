 ...
 * language repo. Runs as the predev/prebuild hook (after sync-docs) so
 * `pnpm dev` and `pnpm build` work without manual setup; `syntaxes/` is
 ...
 *     shiki registration in src/lib/remark-noeta-code.mjs.
 *
 * Two sources, in order of preference (same shape as sync-docs.mjs):
...
try {
  ...
  // Astro's content layer caches rendered markdown and only invalidates on
  // content changes — a grammar-only change would otherwise ship pages with
  // stale highlighting. Bust the store when the synced grammars changed.
  const changed = snapshot().some((g, i) => g !== before[i]);
  if (changed && existsSync(resolve(repoRoot, "node_modules", ".astro"))) {
    console.log("[sync-grammars] Grammars changed; clearing the Astro content-layer cache");

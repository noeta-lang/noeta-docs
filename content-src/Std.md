...
The `std` namespace is the toolchain's built-in module surface — imported explicitly, tree-shaken when unused. This section is the API reference for every `std` module: one page per module family, each listing the module's public functions and types with the exact signatures the compiler enforces.

```noeta
use std.{math, json}           // whole modules → qualified calls: math.sqrt(x)
use std.math.{abs, max}        // selective import → bare calls: abs(-3)
```
